#ifndef JENGINE_UTILS_HASHTABLE_H
#define JENGINE_UTILS_HASHTABLE_H
...
class HashTable {
    ...
    // Hash is the 128bit version of the key
    struct Hash {
        struct { uint64_t upper, lower; };
...
typename HashTable<K,V>::Entry* HashTable<K,V>::GetEntry(const K& key) {
    Hash hashed_key = {};
    Hash128(&key, &hashed_key);
    
    // use lower 64 bits as the index into the entry array
    u64 index = hashed_key.lower % Capacity;
    Entry *entry = nullptr;
    for (;;) {
        entry = &Entries[index];
...
#endif // JENGINE_UTILS_HASHTABLE_H

#if defined(JENGINE_UTILS_HASHTABLE_IMPLEMENTATION)
