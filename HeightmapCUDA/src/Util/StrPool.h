...
void StrPool::Eject(STR_POOL_ID *sid)
{
    ...
    {
        mprinte("Attempted to remove a string from the STRING_POOL with invalid id.\n");
    }
#endif
        ...
        for (u32 i = 0; i < table_cap; ++i)
        {
            ...
            else if (CompareHash64(to_cmp->sid.v, pair.sid.v))
            {
                ...
                if (strncmp(pair.heap, to_cmp->heap, fast_min(pair.heap_size, to_cmp->heap_size)) == 0)
                {
                    ...
                    mprint("Attempting to add a duplicate string. String will be added at next available index.\n");
                    mprint("\tExisting String: %s\n", to_cmp->heap);
                    mprint("\tNew String:      %s\n", pair.heap);
                }
            }
#endif
...
{
    mprinte("Resizing str pair table in StrPool\n");
