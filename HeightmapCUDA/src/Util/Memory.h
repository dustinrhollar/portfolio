...
void memory_free(memory_t *Memory)
{
    ...
    {
        mprinte("Freeing Free List allocator, but not all memory has been freed. There are still %lld allocations with %lld used memory.\n", (*Memory)->NumAllocations, (*Memory)->UsedMemory);
...
void* memory_alloc(memory_t Memory, u64 Size)
{
    ...
    if (Memory->FreeList && (Header = memory_find_free_header(&Memory->FreeList, Size)))
    {
        ...
        if (((char*)Memory->Brkp + AdjSize) <= ((char*)Memory->Start + Memory->Size))
        {
            ...
            // TODO(Dustin): Log
            PlatformFatalError("Requesting more memory than is available!\n");
        }
    }
...
void memory_release(memory_t Memory, void *Ptr)
{
    ...
    {
        PlatformPrintError("Used memory has an underflow!\n");
    }
