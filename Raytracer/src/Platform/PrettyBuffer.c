...
int _format_string(char *buf, int len, char *fmt, va_list list) {
	va_list cpy;
	va_copy(cpy, list);
...
void pb_write(PrettyBuffer *buffer, char *fmt, ...) {
    ...
    i64 chars_needed = _format_string(NULL, 0, fmt, args);
	if (chars_needed + (buffer->current - buffer->start) > buffer->cap) {
		...
		buffer->start = (char*)MemRealloc(buffer->start, buffer->cap);
		buffer->current = buffer->start + offset;
	}
    
    u64 leftover = buffer->cap - (buffer->current - buffer->start);
	chars_needed = _format_string(buffer->current, (i32)leftover, fmt, args);
