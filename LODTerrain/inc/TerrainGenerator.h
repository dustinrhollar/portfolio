...
namespace SimplexNoise {
    ...
    struct NoiseOctaveSimulation {
        ...
        // Octave Information
        int numOctaves;     // Control the number of octaves used
        float persistence;  // Control the roughness of the output
        float low;          // Low output value
        float high;         // Height output value
        float exp;          // Controls the intensity of black to white
    ...
    float* simulateNoise( NoiseOctaveSimulation& octaveInfor );
}
...
namespace Erosion {
    ...
    struct HydraulicErosionSimulation {
        ...
        float *noiseMap;     // temporary noise map
    };
