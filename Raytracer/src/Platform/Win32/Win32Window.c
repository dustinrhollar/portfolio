...
typedef struct HostWnd 
{
    ...
    //HostWndFullscreenCallback fullscreen_callback;
...
void host_wnd_init(struct HostWnd **window, u32 width, u32 height, const char *title)
{
    *window = (HostWnd*)MemAlloc(sizeof(struct HostWnd));
...
void host_wnd_free(struct HostWnd **window)
{
    MemFree(*window);
    *window = 0;
}

void host_wnd_set_callbacks(HostWnd *window, HostWndCallbacks *callbacks)
{
...
void host_wnd_set_active(struct HostWnd *window)
{
    ShowWindow(window->handle, SW_SHOWNORMAL);
    window->is_running = true;
}

bool host_wnd_is_key_pressed(HostWnd *window, MapleKey key)
{
    return (bool)window->keyboard.pressed[(u32)key];
}

bool host_wnd_is_key_released(HostWnd *window, MapleKey key)
{
    return (bool)window->keyboard.released[(u32)key];
}

void host_wnd_get_dims(struct HostWnd *window, u32 *width, u32 *height)
{
    RECT rect;
...
bool host_wnd_msg_loop(HostWnd *window)
{
...
void HostWndSetWindowZorderToTopMost(HostWnd* window, bool is_top)
{
    RECT rect;
...
void HostWndToggleFullscreen(HostWnd* window, IDXGISwapChain* pSwapChain)
{
...
void HostWndD3D12MakeAssociation(host_wnd_t window, IDXGIFactory4 *factory, DeviceResources *graphics_device)
{
    // @ASSUMPTION:
...
bool HostWndD3D12CreateSwapchain(host_wnd_t                              window, 
                ...
                switch (wparam)
                {
                    case VK_RETURN:
                    {
                        if (alt)
                        {
                            case VK_F11: 
                            {
                                HostWndToggleFullscreen(window, g_swapchain);
                                window->callbacks.resize(window->width, window->height);
                            } break;
                        }
                    } break;
